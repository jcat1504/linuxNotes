...
# dynamicUpdates: 0 means false , 1 is true. by default, updates are disabled 
# data and lastUpdateTime are temporarily empty
# data variable keeps downloaded data 
# lastUpdateTime holds last updated time of weather 

#  options: implemented as easy access to app settings 
    # LiveWeather.sh -l Berlin -d -f
    #options are preceded with - except Berlin because it is an argument of option 
    # order of arguments doesn't matter but argument of option must always be placed after option


# $# - at the runtime it contains information about number of given options 
# -gt means greater than..in this case, greater than 0 
# Option variable is getting new value of $1 - first given option 
# below are the options: -l, -d, -f
# $2 is second given option - we use it only if current option is -l(location) b/c there is argument
# option after it
